...
void draw_menu(void) {
    ...
    gotoxy(0, 0); puts("GBDASH");
    gotoxy(0, 1); puts("------");
    for (uint8_t i = 0; i < MAX_LEVELS; i++) {
        gotoxy(1, 3 + i);
        if (i == selected) puts("> ");
        else               puts("  ");
        puts(game_levels[i]->name);
    }
    SHOW_BKG;
    redraw = 0;
}
...
void play_level(uint8_t idx) {
    ...
    DISPLAY_OFF;
    {
        uint8_t _tb = _current_bank;
        SWITCH_ROM(BANK(famidash_chr_tiles));
        if (l->tile_count == 256u) {
            set_bkg_data(0,   128, l->tiles);
            set_bkg_data(128, 128, l->tiles + (128u * 16u));
        } else {
            set_bkg_data(0, (uint8_t)l->tile_count, l->tiles);
        }
        SWITCH_ROM(_tb);
    }
